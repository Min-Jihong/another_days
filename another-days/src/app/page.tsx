...
const Page = () => {
  ...
  return (
    <div className="relative h-screen w-full bg-black" onClick={() => router.replace(paths.home)}>
      <SpaceAnimation />
    </div>
  );
};
    ...
    <>
      ...
      <div ref={textRef} className="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 transform space-x-1 text-3xl font-light text-white">
        {'Another Days'.split('').map((letter, index) => (
          <span
            key={index}
            className="font-space inline-block translate-y-5 text-4xl opacity-0"
            style={{
